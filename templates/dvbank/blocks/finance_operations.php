...
<table class="operations-table-desktop operations-table-desktop--operation">
    <thead>
    ...
    if($historyFinanceData) {
        $moreButton = ($limit && (count($historyFinanceData['historyFinanceArr']) > $limit)) ? true : false;

        $historyFinanceData['historyFinanceArr'] = $limit ? array_slice($historyFinanceData['historyFinanceArr'], 0, $limit) : $historyFinanceData['historyFinanceArr'];

        foreach ($historyFinanceData['historyFinanceArr'] as $historyBlock) {
            ?>
            <tr class="operation">
                <td class="date">
                    <span><?= $historyBlock['historyDate'] ?></span><span><?= $historyBlock['historyTime'] ?></span>
                </td>
                <td class="operation--details"><span><?= $historyBlock['finOperationTitle'] ?></span></td>
                <td class="money">
        ...
        if ($moreButton) { ?>
            <tr class="more-button">
                <td colspan="5" align="center">
                    <a href="/<?= ($tpl->lang . $IB->CONFIG->get("CABINET_OPERATIONS_URL")) ?>">
                        <button class="btn-green">{CABINET-OPERATIONS-MORE}</button>
                    </a>
                </td>
            </tr>
            <?php
...
<ul class="operations-table-mobile">
    ...
    if($historyFinanceData) {
        foreach ($historyFinanceData['historyFinanceArr'] as $historyBlock) {
            $operationStatus = isset($historyBlock['historyItem']->status) ? '{USER-OPERATION-STATUS-' . $historyBlock['historyItem']->status . '}' : "";
            $statusSuffClass = isset($historyBlock['historyItem']->status) ? strtolower($IB->CONSTANTS['UserOperationStatus'][$historyBlock['historyItem']->status]) : "";
            ?>
            <li class="col-xs-12">
                <div class="col-xs-8">
                    <div>
                ...
                <div class="col-xs-4">
                    ...
                    <span class="status-<?= $statusSuffClass ?>"><?= $operationStatus ?></span>
                </div>
            </li>
            <?php
        }
